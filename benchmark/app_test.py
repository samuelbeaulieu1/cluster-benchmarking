...
def seq_test(cluster: str):
    call_endpoint_http(cluster, 1000)

def seq_with_sleep_test(cluster: str):
    call_endpoint_http(cluster, 500)
...
def retrieve_metrics(start_date, end_date, cluster: str):
    elb = get_client("elbv2")
    load_balancer = elb.describe_load_balancers(Names=["c1-c2-alb"])["LoadBalancers"][0]
...
if __name__ == "__main__":
    ...
    threads.append(threading.Thread(target=seq_test, args=["cluster1"]))
    threads.append(threading.Thread(target=seq_with_sleep_test, args=["cluster2"]))
    ...
    retrieve_metrics(start, end, "cluster1")
    retrieve_metrics(start, end, "cluster2")
